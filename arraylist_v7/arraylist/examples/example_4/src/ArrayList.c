...
{   //void**Lista=(pList->pElements)+pList->size;
    int ReturnAux=-1;
...
{  int returnAux = -1;
   int i;
           if(pList!=NULL&&pElement!=NULL)
           {
                for(i=0;i<pList->size;i++)
               {
                if(*(int*)pList->pElements[i]==*((int*)pElement))
                    {
                    return i;
                    }
               }
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    ...
    if(pList!=NULL)
    {
        if (from>=0 && to>=0 && to<=pList->size && from<to)
        {
            for(i=from;i<to;i++)
            {
            Aux=pList->get(pList,i);
		    sublist->add(sublist,Aux);
		    }
           return sublist;
        }
    }
...
{  int returnAux = -1;
   int i;
	...
	for(i=0;i<pList->size;i++)
	{
		if(*(int*)pList->pElements[i]!=*(int*)pList2->pElements[i])
		{
		returnAux=0;
		}
	}
...
int resizeUp(ArrayList* pList)
{
	...
	void* tmp_ptr;
	tmp_ptr = realloc(pList->pElements,sizeof(void*)*(pList->reservedSize+3));
	...
	if (tmp_ptr == NULL) {
		...
		pList->pElements = tmp_ptr;
		pList->reservedSize+=3;
		return 1;
	}
...
int resizeDown(ArrayList* pList)
{
    ...
    if((pList->reservedSize-(pList->size-1)) >10)
    {
    pList->reservedSize=pList->reservedSize-5;
    }
